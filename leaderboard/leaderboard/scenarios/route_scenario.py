...
class RouteScenario(BasicScenario):
    ...
    def _create_test_criteria(self):
        ...
        skip = getattr(self.config, 'skip_criteria', set())

        if 'MinSpeedTest' not in skip:
        ...
        agent_blocked_min_speed = float(os.getenv("B2D_AGENT_BLOCKED_MIN_SPEED", "0.1"))
        agent_blocked_max_time = float(os.getenv("B2D_AGENT_BLOCKED_MAX_TIME", "180.0"))
        criteria.add_child(ActorBlockedTest(
            self.ego_vehicles[0], min_speed=agent_blocked_min_speed, max_time=agent_blocked_max_time,
