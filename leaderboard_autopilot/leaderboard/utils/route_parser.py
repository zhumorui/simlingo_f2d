...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, routes_subset=''):
        ...
        for route in tree.iter("route"):
            ...
            # Parse optional <metrics><skip name="..."/></metrics> block
            metrics_elem = route.find('metrics')
            route_config.skip_criteria = set()
            if metrics_elem is not None:
                for skip_elem in metrics_elem.iter('skip'):
                    name = skip_elem.attrib.get('name', '').strip()
                    if name:
                        route_config.skip_criteria.add(name)
