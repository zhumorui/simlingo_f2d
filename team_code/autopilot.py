...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    self.ignore_coop_left_turn_stopped = int(os.environ.get("COOP_LEFT_TURN_IGNORE_STOPPED", 0)) == 1
    self.ignore_coop_left_turn_off_lights = int(os.environ.get("COOP_LEFT_TURN_IGNORE_OFF_LIGHTS", 0)) == 1
  ...
  def _get_control(self, input_data, plant):
    ...
    planning_vehicles = self._filter_coop_left_turn_planning_vehicles(vehicles)
    ...
    target_speed_route_obstacle, keep_driving, speed_reduced_by_obj = self._manage_route_obstacle_scenarios(
        target_speed, ego_speed, route_wp, planning_vehicles, route_np)
    ...
    if keep_driving:
      ...
      brake, target_speed, speed_reduced_by_obj = self.get_brake_and_target_speed(
          ...
          next_stop_sign, planning_vehicles, actors, target_speed, speed_reduced_by_obj)
  ...
  def _filter_coop_left_turn_planning_vehicles(self, vehicles):
    if not self.ignore_coop_left_turn_stopped:
      return vehicles

    planning_vehicles = []
    for vehicle in vehicles:
      role_name = vehicle.attributes.get("role_name", "")
      if role_name.startswith("coop_left_turn_stopped") or role_name.startswith("coop_left_turn_yielding"):
        continue
      planning_vehicles.append(vehicle)
    return planning_vehicles
  ...
  def ego_agent_affected_by_red_light(self, ego_vehicle_location, ego_vehicle_speed, distance_to_traffic_light,
    ...
    if self.ignore_coop_left_turn_off_lights and next_traffic_light.state == carla.TrafficLightState.Off:
      # Treat disabled lights as unsignalized junctions for coop left-turn data collection.
      return target_speed
